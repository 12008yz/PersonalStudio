...
**В репозитории:** цель, платформа, формат выхода, источники и локализация — в [README.md](README.md), строки UI в `src/ScreenRecorder.App/Strings/`, спеки в `RecordingOutputFormat` / `RecordingSourcesSpec`; каркас `ScreenRecorder.slnx`, `src/ScreenRecorder.App` (WinUI 3).
